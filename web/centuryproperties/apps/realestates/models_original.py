...
#     employee = models.ForeignKey(Employees, on_delete=models.SET_NULL, null=True, related_name='transactions')
#     plot_number = models.CharField(max_length=20, blank=True, default='')
#     date = models.DateTimeField(auto_now_add=True, null=True)
#
#     def calculate_commission(self):
#         total_commission = sum(payment.amount_paid * 0.1 for payment in self.payments.filter(approved=True))
#         return total_commission
#
#     def __str__(self):
#         return f"Transaction {self.id} for {self.client.name}"


class Payment(TruncateTableMixin, models.Model):
    client = models.ForeignKey(Client, on_delete=models.CASCADE, related_name="client_payment", null=True)
